...
if (isset($_POST['reschedule'])) {
    ...
    if ($new_date < $today) {
        ...
        if ($check->fetchColumn() > 0) {
            ...
            $checkPayment = $pdo->prepare("SELECT is_paid FROM bookings WHERE id = ?");
            $checkPayment->execute([$booking_id]);
            $currentBooking = $checkPayment->fetch();
            
            // Tentukan status baru berdasarkan status pembayaran
            // Jika sudah bayar (is_paid = 1), tetap confirmed, hanya perlu konfirmasi reschedule
            // Jika belum bayar, kembali ke pending
            ...
            $stmt->execute([$new_date, $new_time, $therapist_id, $newStatus, $booking_id, $_SESSION['user_id']]);
...
<body class="font-sans text-primary bg-secondary">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16 sm:h-20">
                ...
                <div class="flex items-center gap-2 sm:gap-4">
                    <!-- Back Button -->
                    <a href="member.php" class="mobile-nav-link text-gray-600 hover:text-primary font-medium text-xs sm:text-sm transition-colors">
                    ...
                    <!-- Logout Button -->
                    <a href="../logout.php" class="px-3 sm:px-6 py-2 sm:py-2.5 border border-primary text-primary hover:bg-primary hover:text-white transition-all text-xs sm:text-sm font-medium rounded">
    ...
    <section class="relative pt-20 sm:pt-32 pb-8 sm:pb-12 lg:pb-16 bg-gradient-to-br from-primary via-gray-800 to-primary">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center text-white animate-fade-in-up">
                <h1 class="text-2xl sm:text-4xl lg:text-5xl font-light tracking-tight mb-2 sm:mb-4">Reschedule Booking</h1>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    <div class="bg-gray-50 p-3 sm:p-4 rounded-lg">
                        ...
                        <p class="font-semibold text-primary text-sm sm:text-base"><?php echo $booking['treatment_name']; ?></p>
                        <p class="text-xs sm:text-sm font-bold text-accent mt-1">Rp <?php echo number_format($booking['price'], 0, ',', '.'); ?></p>
                    </div>
                    <div class="bg-gray-50 p-3 sm:p-4 rounded-lg">
                        <p class="text-xs sm:text-sm text-gray-600 mb-1">Terapis</p>
                        <p class="font-semibold text-primary text-sm sm:text-base"><?php echo $booking['therapist_name']; ?></p>
                    </div>
                    <div class="bg-gray-50 p-3 sm:p-4 rounded-lg">
                    ...
                    <div class="bg-gray-50 p-3 sm:p-4 rounded-lg col-span-1 sm:col-span-2">
                        ...
                        <?php if ($booking['is_paid'] == 1): ?>
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-semibold text-green-600 text-sm sm:text-base">LUNAS - Tidak perlu bayar lagi</span>
                            </div>
                        <?php else: ?>
                            <div class="flex items-center gap-2">
            ...
            <div class="bg-yellow-50 border-l-4 border-yellow-500 p-3 sm:p-4 rounded-lg mb-4 sm:mb-6">
                <div class="flex items-start">
                    <i class="fas fa-exclamation-triangle text-yellow-600 mt-0.5 sm:mt-1 mr-2 sm:mr-3 text-sm sm:text-base flex-shrink-0"></i>
                    <div class="text-xs sm:text-sm text-yellow-800">
                        ...
                        <ul class="list-disc list-inside space-y-0.5 sm:space-y-1 text-xs">
                            ...
                            <?php if ($booking['is_paid'] == 1): ?>
                                <li><strong class="text-green-700">✓ Pembayaran Anda tetap valid, tidak perlu bayar lagi</strong></li>
            ...
            <div class="bg-white rounded-lg sm:rounded-xl shadow-md overflow-hidden">
                ...
                <form method="POST" class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div>
                            <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">
                                <i class="fas fa-user-nurse text-accent mr-1 sm:mr-2 text-xs sm:text-sm"></i>Pilih Terapis
                            </label>
                        ...
                        <div>
                            <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">
                                <i class="fas fa-calendar-day text-accent mr-1 sm:mr-2 text-xs sm:text-sm"></i>Tanggal Baru
                            </label>
                            <input type="date" 
                                   name="new_date" 
                    ...
                    <div>
                        <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">
                            <i class="fas fa-clock text-accent mr-1 sm:mr-2 text-xs sm:text-sm"></i>Jam Baru (09:00 - 21:00)
                        </label>
            ...
            <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-8 sm:p-12 text-center">
                ...
                <a href="member.php" class="inline-block mt-3 sm:mt-4 text-accent hover:text-primary font-semibold text-sm sm:text-base">
